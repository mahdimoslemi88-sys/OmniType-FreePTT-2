...
pub struct Toast {
    ...
    pub(crate) value: f32,
    /// Optional per-toast width cap (beats the channel-wide
    /// `Toasts::with_max_width`); `None` = fully automatic width.
...
impl Toast {
    ...
    /// Seeds the card's first-frame width (slide-in animation start and
    /// stack spacing). The card is re-measured from its real content every
    /// frame, so this is NOT a cap — see [`Toast::set_max_width`].
    pub fn set_width(&mut self, width: f32) -> &mut Self {
        self.width = width;
        self
    }

    /// Caps the rendered card width; longer captions wrap (breaking
    /// unbreakable tokens too) into the remaining width. `None` (default)
    /// keeps the fully automatic, content-based width. Beats the
    /// channel-wide `Toasts::with_max_width` when both are set.
    pub fn set_max_width(&mut self, max_width: Option<f32>) -> &mut Self {
