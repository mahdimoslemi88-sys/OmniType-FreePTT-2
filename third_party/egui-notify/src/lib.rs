...
pub struct Toasts {
    ...
    /// Optional width cap for every toast; `Toast::set_max_width` beats it.
...
impl Toasts {
    ...
    /// `Toast::set_max_width` takes precedence when both are set. Caps
    /// don't change behavior when unset — toasts keep their fully
    /// automatic, content-based width.
...
impl Toasts {
    ...
    pub fn show(&mut self, ctx: &Context) {
        ...
        for (i, toast) in toasts.iter_mut().enumerate() {
            ...
            // Create toast label. With a width cap set, re-layout the
            // caption with `Wrap` into the remaining width (card cap minus
            // padding and the icon/close slots); `break_anywhere` hard-breaks
            // unbreakable tokens (URLs, hashes) too, while hard newlines
            // stay paragraph boundaries. Without a cap this is byte-for-byte
            // the old unwrapped layout.
            ...
            // Capped: count real wrapped rows so the icon/close glyphs size
            // from a single row, exactly like the capless layout.
            let line_count = if cap.is_some() {
                caption_galley.rows.len().max(1)
            } else {
                ...
                if let Some((initial, current)) = toast.duration {
                    if !toast.state.disappearing() {
                        p.line_segment(
                            ...
                            Stroke::new(4_f32, visuals.fg_stroke.color),
                        );
                    }
                }
            }
